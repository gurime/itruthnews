...
// 2. Define Props for the extracted component
...
// 3. Move CommentCard OUTSIDE the main component
const CommentCard = ({
comment,
currentUser,
isReply = false,
...
className="px-4 py-1.5 bg-blue-900 text-white rounded-lg hover:bg-blue-800 transition-colors text-sm font-medium cursor-pointer">
Save
</button>
...
className="px-4 py-1.5 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors text-sm font-medium cursor-pointer">
Cancel
</button>
</div>
</div>
) : (
<p className="mt-2 text-gray-800 whitespace-pre-wrap wrap-break-word">
{comment.content}
</p>
)}
</div>
...
className={`flex items-center gap-1 transition-colors ${!currentUser || isOwner ? "text-gray-400 cursor-not-allowed" : comment.user_has_liked ? "text-blue-900 font-semibold": "text-gray-600 hover:text-blue-900"}`}>
...
className="text-gray-600 hover:text-blue-900 font-medium cursor-pointer"
...
disabled={submitting}
className="px-4 py-1.5 bg-blue-900 cursor-pointer text-white rounded-lg hover:bg-blue-800 transition-colors text-sm font-medium disabled:opacity-50">
...
{/* Recursive Replies - You must pass all props down! */}
...
{!user && "Please sign in to comment"}
</span>
