...
export default function Dashboard() {
  ...
  async function handleArticleClick(e: React.MouseEvent, article: Article) {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      // If guest, ensure backend tracks it before navigation
  ...
  async function trackGuestArticleRead(articleId: number) {
    try {
      await fetch('/api/track-article-read', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ articleId })
      });
      // Note: We don't update local state here because we are navigating away immediately
    } catch (error) {
