...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const [politics, economy, tech] = await Promise.all([
          fetchArticlesByCategory("Politics", 4),
          fetchArticlesByCategory("Economy", 4),
          fetchArticlesByCategory("Technology", 4)
        ]);
  ...
  async function fetchArticlesByCategory(category: string, limit: number = 4) {
    try {
      const { data, error } = await supabase
        .from("dashboard")
        .select("*")
    ...
    <>
      ...
      <div className="container mx-auto p-6">
        ...
        {techArticles.length > 0 && (
          <section className="mt-16">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">Technology</h2>
              <Link href="/tech/artificial-intelligence" className="text-blue-600 hover:text-blue-800 font-semibold flex items-center gap-1">
                View All
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
